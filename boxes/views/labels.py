...
def draw_label(canvas, first_name, last_name, barcode_value, date):
    ...
    draw_centered_string(3.2*inch, date, "Helvetica", 12)

    # Draw the barcode
    barcode = createBarcodeDrawing("Code128", value=barcode_value, barWidth=1, barHeight=0.75*inch, humanReadable=False)
    barcode_width = barcode.width
    barcode.drawOn(canvas, (page_width - barcode_width) / 2.0, 3.75*inch)
    draw_centered_string(3.55*inch, barcode_value, "Helvetica", 10)

    # Draw the logo
    logo_path = os.path.join(settings.STATIC_ROOT, "img/logo.png")
    canvas.drawImage(logo_path, (page_width - 1*inch) / 2, 0.3*inch, width=1*inch, height=1*inch, mask="auto")
...
def generate_label(request):
    ...
    for desc, tracking in packages:
        first_name, last_name = desc.split(" ", 1) if " " in desc else ("", desc)
        draw_label(p, first_name, last_name, tracking, label_date)
        p.showPage()
