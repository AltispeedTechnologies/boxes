...
class PackageForm(forms.ModelForm):
    ...
    carrier_id = forms.ChoiceField(label="Carrier",
                                   choices=Carrier.objects.values_list("id", "name").distinct(),
                                   widget=forms.Select(attrs={"class": "form-control select-select2"}))
    account_id = forms.ChoiceField(label="Customer",
                                   choices=Account.objects.values_list("id", "description").distinct(),
                                   widget=forms.Select(attrs={"class": "form-control select-select2"}))
    package_type_id = forms.ChoiceField(label="Type",
                                        choices=PackageType.objects.values_list("id", "description").distinct(),
                                        widget=forms.Select(attrs={"class": "form-control select-select2"}))
